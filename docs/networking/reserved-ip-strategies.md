...
Before this change, the bootstrap layer (subnet carving, infra-role slots, state outputs) and the vault layer (per-workload-subnet mgmt/ocf reserved-IP tables) derived their reserved-IP bands independently, and could disagree. The historical PVE `ocfp` role widened its available band from a `total/2 - 3` computation tied to each subnet's own size, while vault's mgmt tier kept a fixed window; the two could land on different numbers for the same subnet. That mismatch caused the historical PVE band-widening bug.
...
**Subnet identity is recorded, not inferred from names.** Every subnet-creation path persists each subnet's role and workload position at creation time: `role` and `workload_index` resource properties, plus `subnet_<name>_role` and `subnet_<name>_index` state outputs (the index output only for subnets holding a workload position — the infra subnet has none). Consumers that need a subnet's position — Layer A slot resolution, the bastion-subnet lookup, and the `ocfp endpoints`/`ocfp lb`/`ocfp ssh config` lookups below — read these records first and parse the `-ocfp-<n>` name shape only for states predating them. Operator-named subnets (an explicit `network.subnets` list with arbitrary names) therefore resolve pinned statics, reserved tokens, and ssh hosts exactly like default-named ones.
...
The new values are correct. Layer B has placed those four roles at offsets 10, 18, 19, and 17 all along, and its tables are byte-unchanged by this branch; the old Layer A writer hand-coded a separate set of offsets for the `ocfp-1`/`ocfp-2` special case and emitted stale values that disagreed with the records the kits read. No VM moves and no address is at risk — a state output that was wrong is now right — but an operator diffing state outputs across the apply will see these four values move, so this is not a purely additive change.

**Downstream lookups.**

- `ocfp endpoints`' `findReservedIP` helper (`internal/commands/endpoints_collect.go`) collects every `reserved_*_<role>_ip` output. When the bloc's resolved strategy pins the role to a single workload-subnet index (`Layout.PinnedWorkloadIndex`), the key on that index's subnet wins — the subnet identified by its recorded `subnet_<name>_index` output, with the `<bloc>-ocfp-<idx>` name shape as the fallback for states predating those records — so `doomsday_ip` and `shout_ip` resolve from `ocfp-1` and `ocfp_ui_ip` from `ocfp-2` under `spanning`, matching the vault record the kits actually read, even when a stale pre-migration key survives on a lower-sorted subnet. For unpinned roles, or when no strategy resolves, the keys are sorted and the first is used. Under `wide`/`compact` nothing is pinned, so every role takes the sorted-first path and resolves to `ocfp-0`'s address; the kits read a fixed index's vault record (`kits/doomsday` and `kits/shout` both read `/net/subnets/ocfp-1/reserved-ips` unconditionally), so on those strategies the command can still report an address the deployment does not use.

- `ocfp lb add-service`'s `reserved:<key>[:index]` token (`ResolveReservedIP`, `internal/commands/lb.go`) defaults to index 0. `reserved:doomsday_ip`, `reserved:shout_ip`, and `reserved:ocfp_ui_ip` previously failed with `ErrOutputNotFound` because none of those keys existed on `ocfp-0`; under `wide`/`compact` they now return `ocfp-0`'s address instead of erroring. No working configuration regresses — the bare token errored before, so nothing could depend on it — but the silent wrong answer replaces a loud failure, and an operator who wants the address the kits use must name the index explicitly (`reserved:doomsday_ip:1`, `reserved:ocfp_ui_ip:2`). The token's index resolves through the same `subnet_<name>_index` records before the legacy key shape, so these tokens work on operator-named subnets too.

Under `spanning` both lookups are correct: each pinned role's key exists at exactly one index (and `findReservedIP` prefers that index even when stale extras exist), and the bare `reserved:<key>` token still fails loudly for a role that is not pinned to `ocfp-0`, which is the right answer rather than a wrong address.
...
Bootstrap validates on every subnet-creation path. The PVE carve, the STACKIT triple carve, and the STACKIT single carve each call `ValidateSubnetSet` before recording any subnet, and AWS's default VPC subnet split validates the CIDRs of the subnets it is about to create before calling the provider — so a mismatch on any path costs nothing: the bloc is rejected with no partial state written and no cloud subnet created. The AWS check counts every subnet entry — a subnet exists whether or not its CIDR is known yet — and size-checks only the entries that carry one, so provider-assigned (CIDR-less) addressing cannot dodge the count floor; the vault layer's own pre-write validation remains the enforcement of record behind it.
