...
- `ocfp endpoints`' `findReservedIP` helper (`internal/commands/endpoints_collect.go`) collects every `reserved_*_<role>_ip` output. When the bloc's resolved strategy pins the role to a single workload-subnet index (`Layout.PinnedWorkloadIndex`), the key on that index's subnet wins — so `doomsday_ip` and `shout_ip` resolve from `ocfp-1` and `ocfp_ui_ip` from `ocfp-2` under `spanning`, matching the vault record the kits actually read, even when a stale pre-migration key survives on a lower-sorted subnet. For unpinned roles, or when no strategy resolves, the keys are sorted and the first is used. Under `wide`/`compact` nothing is pinned, so every role takes the sorted-first path and resolves to `ocfp-0`'s address; the kits read a fixed index's vault record (`kits/doomsday` and `kits/shout` both read `/net/subnets/ocfp-1/reserved-ips` unconditionally), so on those strategies the command can still report an address the deployment does not use.
