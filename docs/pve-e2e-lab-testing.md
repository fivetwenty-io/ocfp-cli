...
|-------|---------|--------|------------------------|
| 0 Pre-flight | 2026-05-31 | ✅ PASS | Tools OK (`ocfp --version` not `version`; bastion tailscale IP is 100.96.150.97 not doc's 100.119.134.76). Config map-form, genesis-community/genesis@v3.2.x-dev (the CLI default has since moved to RubidiumStudios/genesis). CPI sha1 ✓. `bootstrap --artifacts` & `init pg` both exist. Vault `:8201` reachable. |
...
| Stratos (console) | ocf | 2026-09-14 | ✅ PASS | CF app in `system`/`stratos` space; routes `console.apps.ocf` and `console.system.ocf` | Stratos **v5.5.3** (stable, native CC **v3**) pushed from the published `stratos-cf-v5.5.3.zip` on `binary_buildpack`, via the cf kit `stratos-integration` feature + `genesis <env> do stratos deploy`. Retires the June 2026 in-platform `develop` build and everything it needed: no 16G diego cell, no `maximum_app_disk_in_mb` overrides, no build-info.ts, no `UI_PATH` fix. Eleven addon defects fixed upstream on `cf-genesis-kit` branch `stratos-5x-repair` (PR #239). Colocated `stratos` DB on the CF postgres; `stratos-api`, `stratos-db`, and `stratos-app-ssh` egress ASGs. Feature + version pin now COMMITTED in the env file, which is what June lacked. `https://console.apps.ocf.wayneeseguin.lab.fivetwenty.io` and `https://console.system.ocf.wayneeseguin.lab.fivetwenty.io` both HTTP 200, `/pp/v1/version` `v5.5.3`, app `1/1 running`, clean-slate re-run verified; UAA client carries a sign-in callback for both hostnames. See §8.4. |
...
Ask for the console and for jetstream's version endpoint on both hostnames, and expect 200 from all four. Check both, because only the apps-domain route comes from the push manifest and the system-domain one is mapped in a separate step afterwards. On 2026-09-14 `/pp/v1/version` returned `v5.5.3` built on 2026-09-09, and the app sat at `1/1 running` and ready in the `system`/`stratos` space. Sign in through UAA as well, since the console is pushed with `SSO_LOGIN` set and has no local admin account to fall back on. You can check the client without a browser. Take a client-credentials token for the `admin` client, ask UAA for `GET /oauth/clients/stratos_client`, and read the `redirect_uri` list back. Both console hostnames should be in it. Do not settle for calling `/oauth/authorize` and watching for a 302 to the login page, because UAA validates the redirect URI after the user authenticates and hands an unregistered URI exactly the same 302. If the client is wrong you are locked out of the interface entirely and will have to work from `cf logs`.

The console answers on two names. Its app route is `console.apps.ocf.<bloc>.lab.fivetwenty.io`, and the addon maps a second route at `console.system.ocf.<bloc>.lab.fivetwenty.io`, which is where most people look first, since `shield`, `blacksmith`, `doomsday`, and `concourse` all sit under `*.system.ocf`. Those four are haproxy-routed BOSH deployments and Stratos is an ordinary CF app, so the system-domain name is a second route on the app rather than a change to the ingress. The mapping is allowed because the system domain is a private domain belonging to the `system` org, and the console lives in that org's `stratos` space. UAA holds a sign-in callback for both names. That part matters, because a route SSO will not redirect back to leaves you with a console you can load and cannot sign in to. Before 2026-09-14 only the apps-domain route existed, and asking for `console.system.ocf` got a gorouter 404 that looked exactly like a console that was down.
