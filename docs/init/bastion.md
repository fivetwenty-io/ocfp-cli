...
### pmx Context (PVE only)

On a PVE bloc, the `pmx_context` phase runs right after `vault_populate`, while the inception vault is still reachable, and builds the bastion's `pmx` context from the PVE CPI credentials that phase just wrote to `secret/config/<bloc>/mgmt/cpi/pve`. Before this phase, an operator had to build that context by hand with `safe get` and a long `pmx ctx add` command; the phase now does it for them on every init, so a fresh bastion (or a re-run with `--force`) always has a working, authenticated `pmx` context named `<bloc>-cpi` and selected as the active one. The phase is a no-op, logged rather than failed, on every provider other than PVE.

The script reads `host`, `node`, `token_id`, and `token_secret` from the CPI record with `safe get`, using the same vault path builder that wrote the record, so the bloc name and environment segment are never hand-spelled in either place. It fails the phase with a clear error naming the missing key if any of those four are absent, since `vault_populate` must have run first. It also reads `port` and `verify_ssl` the same way, falling back to `8006` and to the bloc config's `verify_ssl` value (or `false`) when an older vault record predates those keys. `token_id`, which vault stores in the form `user@realm!tokenname`, is split on `!` with `cut` into the `--username` and `--token-id` values `pmx ctx add` expects, exactly as an operator's manual recipe would.

`token_secret` reaches `pmx ctx add --secret` straight from a `safe get` command substitution and is never assigned to a named variable, so it cannot end up in a log line or a debug trace; the bastion scripts never run under `set -x` for the same reason. The `pmx ctx add` call always carries `--force` and `--select`, so a re-run replaces the context instead of failing with "already exists," and `--insecure` is added only when the resolved `verify_ssl` is not `true`. Once the context is written, the phase checks that `pmx`'s config file, `~/.config/pmx/config.yml` by default, is mode `0600`, and tightens it if it is not, since that file holds every context's token secret in plaintext. It finishes by confirming the new context appears in `pmx ctx list` and that `pmx version --context <bloc>-cpi` authenticates against the live PVE API, failing the phase if either check does not confirm the context works.
