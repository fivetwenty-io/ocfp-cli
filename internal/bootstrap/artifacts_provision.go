...
func (m *Manager) provisionArtifactsOverSSH(ctx context.Context, conn artifactsProvisionConn, script string) error {
	...
	cmd := exec.CommandContext(runCtx, "ssh", buildArtifactsProvisionSSHArgs(conn)...) // #nosec G204 -- args are CLI/config-constructed, not user-tainted
...
func (m *Manager) waitArtifactsSSHReady(ctx context.Context, conn artifactsProvisionConn) error {
	...
	for {
		...
		err := exec.CommandContext(probeCtx, "ssh", artifactsSSHArgs(conn, "true")...).Run() // #nosec G204 -- args are CLI/config-constructed, not user-tainted
