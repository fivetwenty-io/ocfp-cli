...
func (m *Manager) createVolumes(ctx context.Context) error {
	...
	volumes := []struct {
		...
		{m.options.BlocName + "-bastion-root", rootDiskSize, "gp3"},
		{m.options.BlocName + "-bastion-data", dataDiskSize, "gp3"},
	}
...
func (m *Manager) processVolume(ctx context.Context, computeMgr cpi.ComputeManager, storageMgr cpi.StorageManager, name string, sizeGB int, vType string) error {
	...
	availabilityZone, err := m.determineVolumeAZ()
	if err != nil {
		return err
	}

	handled, err := m.handleExistingCloudVolume(ctx, storageMgr, name, sizeGB, vType, availabilityZone)
	if err != nil || handled {
		return err
	}
...
// determineVolumeAZ determines the availability zone for a volume based on the provider.
func (m *Manager) determineVolumeAZ() (string, error) {
	if strings.EqualFold(m.options.Provider, "stackit") {
		return m.options.Region + "-1", nil
	}

	if strings.EqualFold(m.options.Provider, "aws") {
		return m.determineAWSVolumeAZ()
	}

	return "", nil
}

// determineAWSVolumeAZ determines the AWS availability zone for a volume.
func (m *Manager) determineAWSVolumeAZ() (string, error) {
	ocfpSubnetName := m.options.BlocName + "-ocfp-0"

	// Try to get AZ from subnet output
	azOutput, err := m.stateManager.GetOutput("subnet_" + ocfpSubnetName + "_az")
	if err == nil && azOutput != nil {
		if az, ok := azOutput.(string); ok && az != "" {
			return az, nil
		}
	}

	// Fallback: try to get from subnet resource properties
	subnetRes, err := m.stateManager.GetResource("subnet", ocfpSubnetName)
	if err == nil && subnetRes != nil {
		if az, ok := subnetRes.Properties["availability_zone"].(string); ok && az != "" {
			return az, nil
		}
	}

	// Final fallback: use first AZ in region
	az := m.options.Region + "a"
	logger.Warnf("Could not determine AZ from subnet, using default: %s", az)

	return az, nil
}

// handleExistingCloudVolume checks for and handles existing volumes in the cloud provider.
func (m *Manager) handleExistingCloudVolume(ctx context.Context, storageMgr cpi.StorageManager, name string, sizeGB int, vType, availabilityZone string) (bool, error) {
