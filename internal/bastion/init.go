...
func (m *Manager) extractGenesisConfig(genesisConfig config.Genesis) (string, string, string) {
	...
	if version == "" {
		version = config.DefaultGenesisVersionPrefix
	}
...
// gitHubSSHClientConfig renders the bastion's ~/.ssh/config stanza for
// github.com. On port 443 the stanza sends the connection to ssh.github.com,
// which is GitHub's SSH endpoint on the HTTPS port, for sites whose firewall
// inspects outbound port 22.
func gitHubSSHClientConfig(port int) string {
	var b strings.Builder

	b.WriteString("# Auto-generated by OCFP bastion init\n")
	b.WriteString("# Enable SSH agent forwarding for GitHub\n")
	b.WriteString("Host github.com\n")

	if port == config.GitHubSSHPortHTTPS {
...
// gitHubKeyscanCommand builds the ssh-keyscan command that seeds the bastion's
// known_hosts for GitHub. OpenSSH looks a host key up by the resolved HostName
// plus any non-default port, not by the name typed on the command line, so
// with the 443 stanza the entries must be keyed [ssh.github.com]:443.
// ssh-keyscan writes exactly that prefix when it is given -p 443, so the scan
// output is appended as is.
func gitHubKeyscanCommand(port int) string {
	target := "github.com"
	if port == config.GitHubSSHPortHTTPS {
		target = "-p 443 ssh.github.com"
	}

	return "mkdir -p ~/.ssh && ssh-keyscan -t rsa,ecdsa,ed25519 " + target +
		" >> ~/.ssh/known_hosts 2>/dev/null && chmod 600 ~/.ssh/known_hosts"
}
...
func (m *Manager) configureSSHClientForwarding(ctx context.Context) {
	port := m.config.Bastion.EffectiveGitHubSSHPort()

	m.log.Infow("Configuring SSH client config for agent forwarding", "github_ssh_port", port)

	createSSHConfigCmd := fmt.Sprintf(`mkdir -p ~/.ssh && cat > ~/.ssh/config << 'EOF'
%s
EOF
chmod 600 ~/.ssh/config`, gitHubSSHClientConfig(port))
...
func (m *Manager) addGitHubHostKeys(ctx context.Context) {
	...
	m.log.Infow("Adding GitHub host keys to known_hosts", "github_ssh_port", port)

	_, err := m.sshClient.ExecuteCommand(ctx, gitHubKeyscanCommand(port))
	if err != nil {
