...
func printFormattedLines(lines []string, outputFormat string) {
	for _, line := range lines {
		_, _ = fmt.Fprintf(os.Stdout, "%s\n", formatLogLine(line, outputFormat)) // #nosec -- output to stdout, not web context
	}
}
...
func printTrimmedLine(line, outputFormat string) {
	...
	_, _ = fmt.Fprintf(os.Stdout, "%s\n", formatLogLine(trimmedLine, outputFormat)) // #nosec -- output to stdout, not web context
}
...
func printLines(lines []string) {
	for _, line := range lines {
		_, _ = fmt.Fprintf(os.Stdout, "%s\n", line) // #nosec -- output to stdout, not web context
	}
}
