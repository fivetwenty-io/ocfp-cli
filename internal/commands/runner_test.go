...
func TestGetSTACKITCredentialsFromVaultTokenSuccess(t *testing.T) {
	...
	fake.outputs["safe -T test-bloc-mgmt get "+tokenPath] = []byte("mytoken\n")
	restore := installFakeRunner(fake)
	defer restore()
...
func TestGetSTACKITCredentialsFromVaultJSONFallback(t *testing.T) {
	...
	fake.outputs["safe -T test-bloc-mgmt get "+tokenPath] = []byte("")
	fake.outputs["safe -T test-bloc-mgmt get "+jsonPath] = []byte(`{"type":"service_account"}`)
	restore := installFakeRunner(fake)
	defer restore()
...
func TestGetAWSCredentialsFromVaultSuccess(t *testing.T) {
	...
	fake.outputs["safe -T test-bloc-mgmt get "+accessPath] = []byte("AKIAIOSFODNN7EXAMPLE\n")
	fake.outputs["safe -T test-bloc-mgmt get "+secretPath] = []byte("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY\n")
	fake.outputs["safe -T test-bloc-mgmt get "+sessionPath] = []byte("")
	fake.outputs["safe -T test-bloc-mgmt get "+regionPath] = []byte("us-east-1\n")
	restore := installFakeRunner(fake)
	defer restore()
