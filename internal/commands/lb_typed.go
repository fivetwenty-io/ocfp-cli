	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
            ...
            if dryRun {
                ...
                t.Sections = append(t.Sections, ui.Section{Title: "Load Balancer", Headers: []string{"NAME", "TYPE", "PROTOCOL", "PORT"}, Rows: [][]string{{name, "external", protocol, fmt.Sprintf("%d", port)}}})
                desired := map[string]bool{}
                if spec, ok := cfg.LBs[name]; ok && len(spec.Targets) > 0 {
                    for _, tgt := range spec.Targets {
                        ip := tgt
                        if isToken(tgt) {
                            if r, err := resolveTargetIP(blocName, tgt); err == nil { ip = r }
                        }
                        desired[ip] = true
                    }
                } else {
                    backends := []struct{ key, index string }{{"vault_ip", "0"}, {"prometheus_ip", "0"}, {"shield_ip", "0"}}
                    if includeDoomsday { backends = append(backends, struct{ key, index string }{"doomsday_ip", "1"}) }
                ...
                addRows := [][]string{}
                for ip := range desired { if !existing[ip] { addRows = append(addRows, []string{ip, fmt.Sprintf("%d", port)}) } }
                if len(addRows) > 0 { t.Sections = append(t.Sections, ui.Section{Title: "Add", Headers: []string{"IP", "PORT"}, Rows: addRows}) }
                if removeUnused {
                    remRows := [][]string{}
                    for ip := range existing { if !desired[ip] { remRows = append(remRows, []string{ip}) } }
                    if len(remRows) > 0 { t.Sections = append(t.Sections, ui.Section{Title: "Remove", Headers: []string{"IP"}, Rows: remRows}) }
                }
			...
			if spec, ok := cfg.LBs[name]; ok && len(spec.Targets) > 0 {
				...
				if err := reconcileLBFromSpec(ctx, lbMgr, netMgr, loadBalancer, blocName, spec, removeUnused); err != nil {
					return err
				}
			} else {
		...
		RunE: func(cmd *cobra.Command, args []string) error {
            ...
            if dryRun {
                ...
                addPlan := func(lbName, proto string, port int, job string) {
                    t.Sections = append(t.Sections, ui.Section{Title: lbName, Headers: []string{"NAME", "TYPE", "PROTOCOL", "PORT"}, Rows: [][]string{{lbName, "external", proto, fmt.Sprintf("%d", port)}}})
                    desired := map[string]bool{}
                    if spec, ok := cfg.LBs[lbName]; ok && len(spec.Targets) > 0 {
                        for _, tgt := range spec.Targets { ip := tgt; if isToken(tgt) { if r, err := resolveTargetIP(blocName, tgt); err == nil { ip = r } }; desired[ip] = true }
                    } else { for _, ip := range getStatePublicIPsByJob(blocName, job) { desired[ip] = true } }
                    existing := map[string]bool{}
                    ...
                    addRows := [][]string{}; for ip := range desired { if !existing[ip] { addRows = append(addRows, []string{ip, fmt.Sprintf("%d", port)}) } }
                    if len(addRows) > 0 { t.Sections = append(t.Sections, ui.Section{Title: fmt.Sprintf("Add (%s)", lbName), Headers: []string{"IP", "PORT"}, Rows: addRows}) }
                    if removeUnused { remRows := [][]string{}; for ip := range existing { if !desired[ip] { remRows = append(remRows, []string{ip}) } }; if len(remRows) > 0 { t.Sections = append(t.Sections, ui.Section{Title: fmt.Sprintf("Remove (%s)", lbName), Headers: []string{"IP"}, Rows: remRows}) } }
                }
                if createHTTP { addPlan(fmt.Sprintf("%s-80", namePrefix), "http", httpPort, "router") }
                if createHTTPS { addPlan(fmt.Sprintf("%s-443", namePrefix), "https", httpsPort, "router") }
                if output == "" { output = "table" }
                return ui.Render(t, strings.ToLower(output))
            }
			...
			if createHTTP {
				...
				if spec, ok := cfg.LBs[name]; ok && len(spec.Targets) > 0 {
					...
					if err := reconcileLBIPs(ctx, lbMgr, netMgr, loadBalancer, desired, httpPort, removeUnused); err != nil {
						return err
					}
				}
			...
			if createHTTPS {
				...
				if spec, ok := cfg.LBs[name]; ok && len(spec.Targets) > 0 {
					...
					desired := getStatePublicIPsByJob(blocName, "router")
					if err := reconcileLBIPs(ctx, lbMgr, netMgr, loadBalancer, desired, httpsPort, removeUnused); err != nil {
						return err
					}
				}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
            ...
            if dryRun {
                ...
                t.Sections = append(t.Sections, ui.Section{Title: "Load Balancer", Headers: []string{"NAME", "TYPE", "PROTOCOL", "PORT"}, Rows: [][]string{{name, "external", "tcp", fmt.Sprintf("%d", port)}}})
                desired := map[string]bool{}
                if spec, ok := cfg.LBs[name]; ok && len(spec.Targets) > 0 {
                    for _, tgt := range spec.Targets { ip := tgt; if isToken(tgt) { if r, err := resolveTargetIP(blocName, tgt); err == nil { ip = r } }; desired[ip] = true }
                } else { for _, ip := range getStatePublicIPsByJob(blocName, "tcp-router") { desired[ip] = true } }
                existing := map[string]bool{}
                if lb, err := netMgr.GetLoadBalancer(ctx, name); err == nil && lb != nil { if pools, err := netMgr.GetBackendPools(ctx, lb.ID); err == nil && len(pools) > 0 { rows := [][]string{}; for _, m := range pools[0].Members { existing[m.IPAddress] = true; rows = append(rows, []string{m.IPAddress, fmt.Sprintf("%d", m.Port)}) }; t.Sections = append(t.Sections, ui.Section{Title: "Current Members", Headers: []string{"IP", "PORT"}, Rows: rows}) } }
                addRows := [][]string{}; for ip := range desired { if !existing[ip] { addRows = append(addRows, []string{ip, fmt.Sprintf("%d", port)}) } }; if len(addRows) > 0 { t.Sections = append(t.Sections, ui.Section{Title: "Add", Headers: []string{"IP", "PORT"}, Rows: addRows}) }
			...
			if spec, ok := cfg.LBs[name]; ok && len(spec.Targets) > 0 {
				...
				if err := reconcileLBIPs(ctx, lbMgr, netMgr, loadBalancer, desired, port, removeUnused); err != nil {
					return err
				}
			}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
            ...
            if dryRun {
                ...
                t.Sections = append(t.Sections, ui.Section{Title: "Load Balancer", Headers: []string{"NAME", "TYPE", "PROTOCOL", "PORT"}, Rows: [][]string{{name, "external", "tcp", fmt.Sprintf("%d", port)}}})
                desired := map[string]bool{}
                if spec, ok := cfg.LBs[name]; ok && len(spec.Targets) > 0 { for _, tgt := range spec.Targets { ip := tgt; if isToken(tgt) { if r, err := resolveTargetIP(blocName, tgt); err == nil { ip = r } }; desired[ip] = true } } else { for _, ip := range getStatePublicIPsByJob(blocName, "cf-ssh") { desired[ip] = true } }
                existing := map[string]bool{}
                if lb, err := netMgr.GetLoadBalancer(ctx, name); err == nil && lb != nil { if pools, err := netMgr.GetBackendPools(ctx, lb.ID); err == nil && len(pools) > 0 { rows := [][]string{}; for _, m := range pools[0].Members { existing[m.IPAddress] = true; rows = append(rows, []string{m.IPAddress, fmt.Sprintf("%d", m.Port)}) }; t.Sections = append(t.Sections, ui.Section{Title: "Current Members", Headers: []string{"IP", "PORT"}, Rows: rows}) } }
                addRows := [][]string{}; for ip := range desired { if !existing[ip] { addRows = append(addRows, []string{ip, fmt.Sprintf("%d", port)}) } }; if len(addRows) > 0 { t.Sections = append(t.Sections, ui.Section{Title: "Add", Headers: []string{"IP", "PORT"}, Rows: addRows}) }
                if removeUnused { remRows := [][]string{}; for ip := range existing { if !desired[ip] { remRows = append(remRows, []string{ip}) } }; if len(remRows) > 0 { t.Sections = append(t.Sections, ui.Section{Title: "Remove", Headers: []string{"IP"}, Rows: remRows}) } }
