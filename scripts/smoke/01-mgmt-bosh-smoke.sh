...
# 01-mgmt-bosh-smoke.sh — Verify wayne mgmt-BOSH director health.
#
...
# Genesis env: ocfp-pve-wayne-mgmt  Director IP: 10.64.64.10

# ---------------------------------------------------------------------------
# Configuration
# ---------------------------------------------------------------------------

BOSH_ENV_NAME="${BOSH_ENV_NAME:-wayne-mgmt}"
BOSH_DIRECTOR_IP="${BOSH_DIRECTOR_IP:-10.64.64.10}"
VAULT_EXODUS_BASE="${VAULT_EXODUS_BASE:-secret/exodus/ocfp-pve-wayne-mgmt/bosh}"
...
log_info "=== mgmt-BOSH smoke test — $(date -u '+%Y-%m-%dT%H:%M:%SZ') ==="
...
# ---------------------------------------------------------------------------
# Step 1: Resolve mgmt-BOSH credentials from vault
# ---------------------------------------------------------------------------
...
CA_CERT_FILE="$(mktemp /tmp/mgmt-bosh-ca-XXXXXX.pem)"
...
if printf '%s\n' "${STEMCELL_OUT}" | grep -qi "0 stemcells\|no stemcells"; then
  log_warn "Step 7: no stemcells uploaded yet — upload a Jammy/Noble stemcell before deploying env-BOSH"
else
...
log_ok "=== mgmt-BOSH smoke test PASSED — director ${BOSH_DIRECTOR_IP} healthy ==="
exit 0
