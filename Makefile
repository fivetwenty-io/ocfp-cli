...
help: ## Show this help message
	...
	@echo "$(WHITE)  make build$(RESET)             - Build binary for current platform"
	@echo "$(WHITE)  make build-linux$(RESET)       - Build Linux binary (amd64)"
	@echo "$(WHITE)  make build-windows$(RESET)     - Build Windows binary (amd64)"
	@echo "$(WHITE)  make build-macos$(RESET)       - Build macOS binary (universal)"
	@echo "$(WHITE)  make build-all$(RESET)         - Build binaries for all platforms"
	@echo "$(WHITE)  make build-release$(RESET)     - Build release binaries with version info"
	@echo ""
	@echo "$(BOLD)$(MAGENTA)Test$(RESET)"
	@echo "$(WHITE)  make test-unit$(RESET)         - Run unit tests with coverage"
	@echo "$(WHITE)  make test-integration$(RESET)  - Run integration tests"
	@echo "$(WHITE)  make test-plugins$(RESET)      - Run plugin tests"
	@echo "$(WHITE)  make tests$(RESET)             - Run all tests"
	@echo ""
	@echo "$(BOLD)$(YELLOW)Check$(RESET)"
	@echo "$(WHITE)  make fmt$(RESET)               - Format code with gofmt"
	@echo "$(WHITE)  make vet$(RESET)               - Run go vet for static analysis"
	@echo "$(WHITE)  make lint$(RESET)              - Run golangci-lint"
	@echo "$(WHITE)  make check$(RESET)             - Run all checks (fmt, vet, lint)"
	@echo ""
	@echo "$(BOLD)$(RED)Security$(RESET)"
	@echo "$(WHITE)  make vulncheck$(RESET)         - Run govulncheck for vulnerabilities"
	@echo "$(WHITE)  make trivy$(RESET)             - Run trivy security scanner"
	@echo "$(WHITE)  make security$(RESET)          - Run all security checks"
	@echo ""
	@echo "$(BOLD)$(CYAN)Development$(RESET)"
	@echo "$(WHITE)  make build-dev$(RESET)         - Build development container"
	@echo "$(WHITE)  make dev$(RESET)               - Run development container"
	@echo "$(WHITE)  make clean$(RESET)             - Clean build artifacts"
	@echo ""
	@echo "$(YELLOW)━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━$(RESET)"
